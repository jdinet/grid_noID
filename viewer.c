...
#define QUIT_KEY 'q'
#define Z_PLUS_KEY 'o'
#define Z_MOINS_KEY 'l'
#define X_LEFT_KEY 'k'
#define X_RIGHT_KEY 'm'


/* - Mouse State Variables - */
int g_vMousePos[2]       = {0, 0};
int g_vAccelAngle[3]     = {0,0,0};
...
void display() {
	...
	glRotatef(g_vAccelAngle[0],0.0,0.0,1.0);
	glRotatef(g_vAccelAngle[1],0.0,1.0,0.0);
	redraw_world();
    glutSwapBuffers();
}

void reshape(int w,int h) {
	perspective_init(w, h);
}
...
void mouse_accel(int x, int y) {
	int vMouseDelta[2] = {x-g_vMousePos[0], y-g_vMousePos[1]};
	g_vAccelAngle[0]+=vMouseDelta[0];
	g_vAccelAngle[1]+=vMouseDelta[1];

	Block *block;
	Q_FOREACH(block, getBlockList(), blockLink) {
	    block->_acc.x = g_vAccelAngle[0];
		block->_acc.z = g_vAccelAngle[1];
    // just for a test
	}
 ...
 */
 ...
 void keyboard_motion(void){

   Block *block;
   Q_FOREACH(block, getBlockList(), blockLink) {
       block->_acc.x = g_vAccelAngle[0];
     block->_acc.z = g_vLandRotate[0]+24;

     block->_acc.y = 10; //Debug
   }
...
void key_button (unsigned char key, int x, int y)
{
	...
	case ACCEL_LOCK_KEY:
		...
		if (accelActive) glutMotionFunc(mouse_accel);
		else glutMotionFunc(mouse_drag);
		break;
	case VIEW_RESET_KEY:
		g_vLandRotate[0]    = 0.0;
		g_vLandRotate[1]    = 0.0;
  ...
  case Z_PLUS_KEY:
    ...
    g_vLandRotate[0] =-40;
    //g_vAccelAngle[1]=g_vAccelAngle[1] - 2;
  ...
  case X_LEFT_KEY:
    ...
    g_vLandRotate[2] =g_vLandRotate[2]+4;
    g_vAccelAngle[0]= g_vAccelAngle[0] + 15;
